...
const TRACKING_PURPOSE =
  'This lets ScribeZero show ads that are more relevant to you. Your recordings are never uploaded, your data is never sold, and the app works exactly the same either way.';
...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...config,
    name: config.name ?? 'ScribeZero',
    slug: config.slug ?? 'scribezero',
    plugins: [

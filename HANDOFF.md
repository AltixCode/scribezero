...
| | |
|---|---|
| Bundle id / package | `com.altixcode.scribezero` |
| App Store Connect app | `6811548775` (version 1.0, `PREPARE_FOR_SUBMISSION`) |
| In-app purchase | `scribezero_pro_lifetime` — ASC id `6811578499`, state `MISSING_METADATA` |
| RevenueCat project | `proj398723ff` |
| RevenueCat apps | iOS `app98a154893b` · Android `app834ba89acc` |
...
- **Interstitial**: app/transcript.tsx — behind the share sheet for a transcript. Pacing lives in `adPolicy.ts`: the first completion
...
1. **Fix the entitlement mismatch — this one ships broken.**
   ...
   rc entitlements create --lookup-key remove_ads \
     --display-name "Pro — Ad-Free & Unlimited" --project-id proj398723ff --json --yes
   rc entitlements attach <entitlementId> <productId> --project-id proj398723ff --json --yes
   ```
...
4. **Upload the IAP review screenshot.** The in-app purchase
   (`scribezero_pro_lifetime`) is localized and priced at $3.99, and sits at
   `MISSING_METADATA` for want of one screenshot:

   ```bash
   asccli iap-review-screenshot upload --iap-id 6811578499 --file <path-to-png>
   ```
...
2. **RevenueCat — Apple credentials.** `app98a154893b` (the App Store app in project
   `proj398723ff`) has no Apple credentials, so App Store purchases cannot be
   validated. This needs an interactive Apple ID sign-in with 2FA:

   ```bash
   rc setup apple app98a154893b
   ```
...
3. **GitHub secrets.** The signing secrets are all present
   ...
   Set each with `gh secret set <KEY> --repo AltixCode/scribezero`. The RevenueCat
   public SDK keys are fetchable — `rc api GET "/projects/proj398723ff/apps/app98a154893b/public_api_keys"`
   — the AdMob ones come from step 1.

4. **The legal URLs are wrong and they 404.** `src/config/legal.ts` points at
   `https://www.hushtunnel.com/legal/scribezero-privacy` — **hushtunnel.com**, a leftover from the HushTunnel
...
5. **Play Console — the app record exists.** `com.altixcode.scribezero` is recognised,
...
../scripts/verify-app.sh scribezero com.altixcode.scribezero   # full build + device verification, both platforms
```
